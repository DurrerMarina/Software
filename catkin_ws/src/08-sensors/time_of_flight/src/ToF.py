...
class ToF():
	...
	def begin(self):
		...
		self.MyBus.write_word_data(self.addr, self.RFD77402_CONFIGURE_B, 0x10FF)				#Set valid pixel. Set MSP430 default config.
		self.MyBus.write_word_data(self.addr, self.RFD77402_CONFIGURE_HW_0, 0x07D0)			#Set saturation threshold = 2,000.
		self.MyBus.write_word_data(self.addr, self.RFD77402_CONFIGURE_HW_1, 0x5008)			#Frequecy = 5. Low level threshold = 8.
		self.MyBus.write_word_data(self.addr, self.RFD77402_CONFIGURE_HW_2, 0xA041)			#Integration time = 10 * (6500-20)/15)+20 = 4.340ms. Plus reserved magic.
		self.MyBus.write_word_data(self.addr, self.RFD77402_CONFIGURE_HW_3, 0x45D4)			#Enable harmonic cancellation. Enable auto adjust of integration time. Plus reserved magic.
	...
	def getCalibrationData(self):
		...
		messages = 0
		while (True):
			if (self.MyBus.read_byte_data(self.addr,self.RFD77402_ICSR)& (1<<5)) == 0:		#Mailbox interrupt is cleared
				break
